---
title: "Comparison of occupancy models on an empirical example"
subtitle": "Presence of the lynx (*Lynx lynx*) in the Ain county, France"
author: "Léa Pautrel"
...
<style type="text/css">
  ...
  blockquote {
    padding: .5em;
    background-color: #BED62F4D;
    text-align: center;
    font-size: 1.1em;
    border-radius: 50px; 
  }
  p, h1, h2, h3, h4, h5, h6, ul, ol {font-family: "Sansation";}
  h1 {background: #58A946; color: #FFFFFF; padding:5px; border-radius: 5px; font-weight:bold; text-align: center;}
  h2 {background: #7FC241; color: #FFFFFF; padding:5px; border-radius: 5px; font-weight:bold; text-align: center;}
  h3 {color: #7FC241; font-weight:bold;}
  h4, h5, h6 {color: #BED62F; font-weight:bold;}
  p, ul, ol {color: #6C5F5C; text-align: justify;}
  a {color: #00A695;}
  a:hover {color: #007A6D;}
  code {background-color: #E6E6E6; color: #58A946}
</style>
...
- The total number of camera-trap days is `r sum(allsites$DeploymentDuration)`.
- The lynx was detected `r sum(allsites$NbLynx)` times in total.


Let's see how many detections there are per site and per species.


```{r plot_nb_detec_per_site_and_species_all}
allsites %>% 
...
- `r sum(DayLongDF$PartiallyMonitored)` sessions were dropped because they were incomplete.
- This corresponds to a duration of `r as.character(as.period(as.duration(sum(DayLongDF[DayLongDF$PartiallyMonitored,]$MonitoringTime))))` where sites were monitored that were discarded.
...
- `r sum(WeekLongDF$PartiallyMonitored)` sessions were dropped because they were incomplete.
- This corresponds to a duration of `r as.character(as.period(as.duration(sum(WeekLongDF[WeekLongDF$PartiallyMonitored,]$MonitoringTime))))` where sites were monitored that were discarded.
...
- `r sum(MonthLongDF$PartiallyMonitored)` sessions were dropped because they were incomplete.
- This corresponds to a duration of `r as.character(as.period(as.duration(sum(MonthLongDF[MonthLongDF$PartiallyMonitored,]$MonitoringTime))))` where sites were monitored that were discarded.
...
The model running time is: `r print(aftertime - beforetime)`
...
```{r fit_PP_all, class.source = 'fold-show'}
beforetime = Sys.time()
fitted_PP <- optim(
  # Initial parameters
  par = c(
